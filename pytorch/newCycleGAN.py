...
class CycleGAN(object) :
    def __init__(self, args):
        ...
        self.trainA_dataset = self.trainA_dataset3
        self.trainB_dataset = self.trainA_dataset2
    ...
    def train(self):
        ...
        self.tlab_t = np.array(1, dtype=np.int64) * np.ones((self.batch_size), dtype=np.int64)
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if len(self.trainA_dataset) - idx*self.batch_size < 0:
                    next_start_pos_a = -1
                if len(self.trainB_dataset) - idx * self.batch_size < 0:
                    next_start_pos_b = -1
                ...
                if epoch % 2 == 0:
                    ...
                    self.tlab_o = self.tlab_o * 0
                    self.tlab_t = self.tlab_t * 0 + 1
                    ...
                    self.tlab_o = self.tlab_o * 0 + 1
                    self.tlab_t = self.tlab_t * 0
                ...
                video = np.pad(video, ((0, 0), (3, 3), (3, 3), (3, 3), (0, 0)), 'reflect')
                    ...
                    #video += np.random.randn(self.batch_size, self.num_clips+6, self.img_size+6, self.img_size+6, 3)
                ...
                if True:#(self.black_box and epoch % 2 != 0) or not self.black_box:
                    ...
                    x_fnoise, _ = self.G(x_fake, self.tlab_t)
                    x_cycle = x_fake + x_fnoise.repeat(1, 1, 16, 1, 1)
                    x_cycle = torch.clamp(x_cycle, min=0., max=255.)
                    ...
                    if self.black_box:
                        ...
                        self.cycle_logit = self.B(x_cycle)
                        self.cycle_loss = L1_loss(self.cls_logit, self.cycle_logit)
                        ...
                        self.cycle_logit = self.C(x_cycle)
                        self.cycle_loss = L1_loss(self.cls_logit, self.cycle_logit)
    ...
    def test(self, exe):
        ...
        for idx in range(2):#len(test_files)) : # A -> B
