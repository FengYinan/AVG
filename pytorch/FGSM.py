    ...
    def test(self, exe):
        ...
        for idx in range(len(test_files)) : # A -> B
            ...
            for step in range(all_steps):
                ...
                loss = F.cross_entropy(self.output_real,self.tlab_t)

                loss.backward()
                grad_sign = video.grad.data.cpu().sign().numpy()
                grad_sign = torch.from_numpy(grad_sign).to(self.device).requires_grad_(False)

                x_fake = video - grad_sign * 10
                ...
                noise = (grad_sign * 10).cpu().detach().numpy()
