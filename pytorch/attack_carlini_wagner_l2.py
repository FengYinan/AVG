...
class AttackCarliniWagnerL2:
    ...
    def __init__(self, targeted=True, search_steps=None, max_steps=None, cuda=True, debug=False):
        ...
        self.initial_const = 1e-6  # bumped up from default of .01 in reference code
        self.binary_search_steps = 1
        self.repeat = self.binary_search_steps >= 10
        self.max_steps = max_steps or 2000
    ...
    def _optimize(self, optimizer, model, input_var, modifier_var, target_var, scale_const_var, input_orig=None):
        ...
        if self.clamp_fn == 'tanh':
            input_adv = tanh_rescale(modifier_var*10 + input_var, self.clip_min, self.clip_max)
        else:
            input_adv = torch.clamp(modifier_var*10 + input_var, self.clip_min, self.clip_max)
    ...
    def run(self, model, input, target, batch_idx=0):
        ...
        optimizer = optim.Adam([modifier_var], lr=0.0001)
