...
class CycleGAN(object) :
    ...
    def build_model(self):
        ...
        with tf.device('/cpu:0'):
            self.logit_original = self.classify(self.domain_A/255.)
        ...
        with tf.device('/cpu:0'):
            self.cls_logit = self.classify(x_fake/255., reuse=True)
        ...
        self.test_fake = self.test_video + tf.tile(self.test_fake_noise, multiples=[1, 16, 1, 1, 1])
        self.test_fake = tf.minimum(tf.maximum(self.test_fake , 0.), 255.)

        with tf.device('/cpu:0'):
            self.test_logit = self.classify(self.test_fake/255., reuse=True)
                ...
                if (self.black_box and epoch % 2 != 0) or not self.black_box:
                    video, next_start_pos_a, read_dirnames, _, _, _ = \
                        input_data.read_clip_and_label(self.trainA_dataset, self.batch_size,
                                                       num_frames_per_clip=self.num_clips, start_pos=next_start_pos_a, crop_size=self.img_size)
                ...
                if self.black_box and epoch % 2 == 0:
                    video, next_start_pos_b, read_dirnames, _, _, _ = \
                        input_data.read_clip_and_label(self.trainB_dataset, self.batch_size,
                                                       ...
                                                       start_pos=next_start_pos_b, crop_size=self.img_size)
    ...
    def test_small(self,exe):
        ...
        classify_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='RGB')
            ...
            for step in range(2):
                sample_image2, _, _, _, _, _ = \
                    input_data.read_clip_and_label(test_files, 1, num_frames_per_clip=self.num_clips,
                                                   start_pos=next_start_pos, crop_size=self.img_size)
